...
module.exports = function(app) {
  ...
  app.post('/api/artists', function(req, res) {
    ...
    request(artistQuery, function(err, response, body){
      if (!err && response.statusCode == 200) {
        ...
        // GET ARTIST ALBUMS
          var tracksQuery = "https://api.spotify.com/v1/artists/" + artistId + "/albums";
  ...
  app.post('/api/songs', function(req, res) {
    var formArtist = req.body.artist;
    var formTrack = req.body.track;
    console.log("form: ", formArtist, formTrack);

    // send artist and track name into lyric api to get track id
    var trackIdQuery = "http://api.musixmatch.com/ws/1.1/matcher.track.get?apikey=" + MM_API_KEY + "&q_track=" + formTrack + "&q_artist=" + formArtist + "&f_has_lyrics=1";
    request(trackIdQuery, function(err, response, body) {
      if (!err && response.statusCode == 200) {
        var trackData = JSON.parse(body);
        // get musix track id
        ...
        // send songId into lyrics api to get lyrics
